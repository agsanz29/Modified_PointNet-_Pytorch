...
def main(metrics_txt, classes_txt, colours, args):
    ...
    add_dir = os.path.join(r'log\classification', folder)

    folder_save = os.path.join(BASE_DIR,os.path.join(add_dir,'output_images'))
    if not os.path.exists(folder_save):
        os.makedirs(folder_save)

    folder_names = os.path.join(BASE_DIR,os.path.join(r'data',folder))
    object_folder_names = os.path.join(BASE_DIR, os.path.join(r'data',object_folder))
    names_path = os.path.join(object_folder_names,classes_txt)
    with open(names_path, 'r', encoding='utf-8') as text_file:
        classes = [line.strip() for line in text_file.readlines()]

    logs_path = os.path.join(BASE_DIR,os.path.join(add_dir,'logs'))
